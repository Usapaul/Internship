...
filename = 'for_robust_mag_err_0p02_22k.dat'
...
#col_u_sdss = 'z_mean_psf_mag'
col_g_sdss = 'g_mag'
col_r_sdss = 'r_mag'
col_i_sdss = 'i_mag'
#col_z_sdss = 'y_mean_psf_mag'
...
print('weights ',results.weights.shape)
print('resid ',results.resid.shape)

resTable = Table([tbl['source_id'],results.weights,results.resid],
					names=('source_id','weight','resid'),dtype=('i8','f4','f4'))

ascii.write(resTable,'check_weights_resid_TrimmedMean.dat',
			format='commented_header',overwrite=True)


count = 0
for i in range(len(results.weights)):
	if results.weights[i] > 1e-5:
		count += 1
